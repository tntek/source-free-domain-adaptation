...
def lr_scheduler(optimizer, iter_num, max_iter, gamma=10, power=0.75, cfg=None):
    decay = (1 + gamma * iter_num / (max_iter*cfg.OPTIM.WD)) ** (-power)
...
def test_time_tuning(model, inputs, optimizer, cfg, target_output):
    ...
    for j in range(cfg.ProDU.TTA_STEPS):
...
def MI_perturbation(cfg, theta_v, output_t, P_EPSILON, sign='max'):
    
    if sign not in ('max', 'min'):
        raise ValueError('The value of sign must be \'max\' or \'min\'.')

    theta_v = theta_v.detach()
    output_t = output_t.detach()

    p_t = F.softmax(output_t, dim=1)

    delta = torch.zeros_like(theta_v, requires_grad=True)
    optimizer = torch.optim.Adam([delta], lr=cfg.ProDU.DELTA_LR)
    for _ in range(cfg.ProDU.P_STEPS):
        optimizer.zero_grad()

        perturbed_logits = theta_v + delta
        p_v = F.softmax(perturbed_logits, dim=1)

        mi_loss = IID_losses.IID_loss(p_t, p_v)
        if sign == 'min':
            mi_loss *= -1

        overflow = torch.clamp(torch.abs(delta) - P_EPSILON, min=0.0)
        # 1.0 for best
        reg_loss =  cfg.ProDU.INNER * torch.sum(overflow)

        total_loss = reg_loss + mi_loss

        total_loss.backward()
        optimizer.step()

    # with torch.no_grad():
    #     delta.data = delta.data.clamp(-args.p_epsilon, args.p_epsilon)
    
    perturbed_logits_v = (theta_v + delta).detach()
    return perturbed_logits_v

def train_target(cfg):
    text_inputs = clip_pre_text(cfg)
    dset_loaders = data_load(cfg)
    model = get_coop(cfg.ProDU.ARCH, cfg.SETTING.DATASET, int(cfg.GPU_ID), cfg.ProDU.N_CTX, cfg.ProDU.CTX_INIT)
    ...
    model.reset_classnames(cfg.classname, cfg.ProDU.ARCH)
    max_iter = cfg.TEST.MAX_EPOCH * len(dset_loaders["target"])
    batch_num =  max_iter // cfg.TEST.MAX_EPOCH
    ...
    p_bank = torch.zeros_like(logtis_bank)
    ...
    with torch.no_grad():
        ...
        for i in range(len(loader)):
            ...
            p_bank[indx] = F.softmax(outputs.detach().clone(), dim=1)
    ...
    while iter_num < max_iter:
        ...
        epoch_num = iter_num // batch_num
        iter_num += 1

        lr_scheduler(optimizer, iter_num=iter_num, max_iter=max_iter, cfg=cfg)
        ...
        t_entropy = -torch.sum(softmax_out*torch.log(softmax_out+1e-10), dim=1)
        m_t_entropy = torch.mean(t_entropy)
        ...
        with torch.no_grad():
            if cfg.SETTING.DATASET=='VISDA-C':
                ...
                logits_s = 1.0
            new_clip = (outputs_test_new - logits_s*logtis_bank[tar_idx].cuda()) + clip_score.cuda()
            raw_clip = clip_score.cuda()

            s_entropy = -torch.sum(p_bank[tar_idx]*torch.log(p_bank[tar_idx]+1e-10), dim=1)
            m_s_entropy = torch.mean(s_entropy)

            res_entropy = m_t_entropy / (m_s_entropy + cfg.ProDU.EPSILON)

            if iter_num == 1:
                max_val = res_entropy
            r_entropy = res_entropy / max_val
            
            W = r_entropy

            clip_score_sm = nn.Softmax(dim=1)(new_clip)

        P_EPSILON = cfg.ProDU.L * (1 - torch.sigmoid(W/cfg.ProDU.R))

        clr_new_clip = MI_perturbation(cfg, raw_clip, outputs_test, P_EPSILON, sign='max')

        max_probs, _ = torch.max(clip_score_sm, dim=1)
        confident_idx = torch.where(max_probs > cfg.ProDU.THRESHOLD+0*(iter_num/max_iter))[0]
        
        _,clip_index_new = torch.max(new_clip, 1)
        clip_score_sm = nn.Softmax(dim=1)(new_clip)
        _,clr_clip_index_new = torch.max(clr_new_clip, 1)
        clr_clip_score_sm = nn.Softmax(dim=1)(clr_new_clip)
        
        iic_loss = IID_losses.IID_loss(softmax_out, clip_score_sm)
        p_iic_loss = IID_losses.IID_loss(softmax_out, clr_clip_score_sm)
        if cfg.SETTING.DATASET=='VISDA-C':
            iic_loss = cfg.ProDU.OUTER*p_iic_loss
        else:
            iic_loss += cfg.ProDU.OUTER*p_iic_loss
        

        classifier_loss = cfg.ProDU.IIC_PAR * iic_loss
        msoftmax = softmax_out.mean(dim=0)

        if  cfg.SETTING.DATASET=='office':
            gentropy_loss = torch.sum(-msoftmax * torch.log(msoftmax + cfg.ProDU.EPSILON))
        ...
        if  cfg.SETTING.DATASET=='VISDA-C':
            gentropy_loss = torch.sum(-msoftmax * torch.log(msoftmax + cfg.ProDU.EPSILON))
            classifier_loss = classifier_loss - 0 * gentropy_loss
        pred = clip_index_new

        entropy_loss = nn.CrossEntropyLoss()(outputs_test[confident_idx], pred[confident_idx])
        classifier_loss =  cfg.ProDU.GENT_PAR*entropy_loss + classifier_loss
...
def clip_pre_text(cfg):
    ...
    prompt_prefix = cfg.ProDU.CTX_INIT.replace("_"," ")
